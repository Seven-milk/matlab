%copula拟合,为了计算互信息——copula熵，只需要计算其cdf
function [p_best,string_best,aic_best,rho_best,copulaep]=Bestfit_copulapdf(A,B)
...
%计算copula密度函数值
cpdf_clayton=copulapdf('clayton',U,rho_clayton);
cpdf_frank=copulapdf('frank',U,rho_frank);
cpdf_gumbel=copulapdf('gumbel',U,rho_gumbel);
cpdf_gaussian=copulapdf('gaussian',U,rho_gaussian);
cpdf_t=copulapdf('t',U,rho_t,nuhat);
%% -----------------------------------------------参数输入
...
pdf_=[cpdf_clayton,cpdf_frank,cpdf_gumbel,cpdf_gaussian,cpdf_t];
m=[1,1,1,1,2];%参数数目
mse=zeros(5,1);%均方误差
%% ------------------------------------------------ols计算
% ols=zeros(5,1);
